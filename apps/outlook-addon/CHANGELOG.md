...
The version jumps from 0.4.0 to 1.0.0 deliberately. Outlook keys sideloaded and centrally-deployed updates on the manifest's `<Version>`, and `office-addin-manifest` rejects any `<Version>` below 1.0 — so the manifest had always declared a hardcoded `1.0.0.0` while `package.json` said 0.4.0, and nothing kept the two in step. Every release therefore shipped a manifest an admin could not tell apart from the previous one. `package.json` now matches what the manifest has always claimed, `version-packages` syncs the manifest on every bump, and CI refuses a commit where they disagree.

Also in this release:

- The Cryptify host is `storage.postguard.eu`. The released add-in baked `fileshare.postguard.eu`, which no longer resolves, so file sending was broken for every Outlook user while CI was green ([postguard-outlook-addon#132](https://github.com/encryption4all/postguard-outlook-addon/issues/132)). CI now fails when any host the build can bake in does not resolve.
- The shipped manifest allowlists only the origins its own build uses. Previously every build allowlisted the production _and_ staging add-in origins plus two spellings of its own, because the source manifest lists both environments. A production build now refuses outright when an origin it needs is absent, since a missing `AppDomain` breaks the Yivi dialog only at send time.
- CI validates `dist/manifest.xml` — the copy admins sideload — for both the edge and production configurations, and again for the exact artifact attached to a release.
